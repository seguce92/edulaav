    ...
    <div @keydown.esc="toggleModal" v-if="value" :class="value ? '' : 'opacity-0 pointer-events-none'" class="modal fixed inset-0 w-full h-screen flex items-center justify-center z-9999 font-nunito">
        ...
        <div class="modal-container bg-white fixed w-full h-full z-50 overflow-y-auto ">
            <div class="py-2 px-4 border-b border-primary-500 flex items-center sticky top-0 z-50 bg-white">
                ...
                </button> <span class="text-lg ml-3 font-bold">Material</span>
            </div>
            <div class="overflow-y-auto flex flex-wrap md:min-h-modal">
                <div class="w-full md:w-4/5 px-4 py-4">
                    <div class="flex mb-3">
                        <span class="px-4 py-2 whitespace-no-wrap">
                    ...
                    <div v-if="markdown" class="flex">
                        ...
                        <markdown-editor class="w-full" v-model="model.content" :options="options" />
                    </div>
    ...
    methods: {
        ...
        async store () {
            ...
            await this.$http.post('api/lms/tasks', this.model)
                .then((response) => {
                    this.$toast.success("Material registrada exitosamente.")
                    this.initialize()
        ...
        async update (id) {
            ...
            await this.$http.put('api/lms/tasks/'+id, this.model)
                .then((response) => {
                    this.$toast.success("Material actualizado exitosamente.")
                    this.initialize()
                    this.$root.$emit('updateListTask');
                    this.toggleModal()
                }).catch((response) => {
