            ...
            <div>
                <dl>
                    ...
                    <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm leading-5 font-medium text-gray-500">
                            Titulo <span aria-label="Este campo es requerido" data-balloon-pos="right"><i class="fa fa-asterisk text-xss"></i></span>
                        </dt>
                        <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                            <div class="flex flex-wrap -mx-3">
                                <div class="w-full px-3">
...
export default {
    ...
    methods: {
        async store () {
            this.loading = true
            await this.$http.post('api/academic/parallels', this.model)
                .then((response) => {
                    if ( response.status == 201 ) {
                        this.$toast.success("Paralelo registrado exitosamente.")
