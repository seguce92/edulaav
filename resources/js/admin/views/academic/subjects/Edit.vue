        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg max-w-6xl mx-auto">
            <div class="px-4 py-5 border-b border-gray-200 sm:px-6">
                ...
                <p class="mt-1 max-w-2xl text-sm leading-5 text-gray-500">
                    Actualizar información de Paralelo
                </p>
            </div>
            <div>
                <dl>
                            ...
                            <div class="flex flex-wrap -mx-3">
                                <div class="w-full px-3">
                                    <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 uppercase" 
                                        ...
                                        placeholder="EXAMPLE">
                                    <error-field :errors="set_error(errors, 'code')"></error-field>
                                </div>
                            </div>
                        </dd>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm leading-5 font-medium text-gray-500">
                            Nombre <span aria-label="Este campo es requerido" data-balloon-pos="right"><i class="fa fa-asterisk text-xss"></i></span>
                        </dt>
                        <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                            <div class="flex flex-wrap -mx-3">
                                <div class="w-full px-3">
                    ...
                    <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm leading-5 font-medium text-gray-500">
                            Descripción <span aria-label="Este campo es requerido" data-balloon-pos="right"><i class="fa fa-asterisk text-xss"></i></span>
                        </dt>
                        <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                            <div class="flex flex-wrap -mx-3">
                                <div class="w-full px-3">
                                    <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" 
                                        type="text" required v-model="model.description"
                                        placeholder="Descripción">
                                    <error-field :errors="set_error(errors, 'description')"></error-field>
                                </div>
                            </div>
                        </dd>
                    </div>
...
export default {
    ...
    methods: {
        async fetchData (id) {
            this.loading = true
            await this.$http.get('api/academic/subjects/' + id)
                .then((response) => {
        ...
        async store () {
            this.loading = true
            await this.$http.put('api/academic/subjects/'+this.model.id, this.model)
                .then((response) => {
                    if ( response.status == 201 ) {
                        this.$toast.success("Asignatura registrado exitosamente.")
                        this.$router.push({ name: 'grades.show', params:{id:this.model.grade_id} })
                    }
                }).catch((response) => {
