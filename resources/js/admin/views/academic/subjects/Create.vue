                ...
                <dl>
                    ...
                    <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        ...
                        <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                            <div class="flex flex-wrap -mx-3">
                                <div class="w-full px-3">
                                    <textarea class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" 
...
export default {
    ...
    created() {
        this.model.grade_id = this.$route.params.id
    },
    methods: {
        async store () {
            this.loading = true
            await this.$http.post('api/academic/subjects', this.model)
                .then((response) => {
                    if ( response.status == 201 ) {
                        this.$toast.success("Asignatura registrado exitosamente.")
                        this.$router.push({ name: 'grades.show', params:{id:this.$route.params.id} })
                    }
                }).catch((response) => {
