            ...
            <div v-for="(row, index) in data" :key="index">
                <div class="flex h-10 justify-between my-4 items-center border-b border-primary-400">
                    ...
                    <div>
                        ...
                        <router-link :to="{name:'designations.create',params:{id:row.id}}" class="btn-small-outline md:ml-3" aria-label="Nueva Designación" data-balloon-pos="left">
                            <i class="fa fa-plus"></i>
                        </router-link>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <article v-for="(des, i) in row.designations" :key="i"
                        ...
                        <div class="flex flex-col px-4 py-3 font-nunito">
                            <h3 class="font-bold text-lg relative items-center">
                                <a class="cursor-pointer hover:underline" @click="showDesignation(des.id)">{{ des.title }}</a>
                            ...
                            <h4 class="text-xs text-gray-800 uppercase font-bold tracking-tighter">{{ des.group }}</h4>
                            <div class="flex items-center py-2 text-gray-800">
                                <i class="fa fa-user text-sm mr-2"></i>
                                <span class="text-sm font-nunito tracking-tighter">{{ des.teacher }}</span>
                            </div>
                        </div>
                    </article>
                </div>
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-2">
                        <p class="text-sm leading-5 text-gray-500">
                            ...
                            Todos sus datos serán eliminados permanentemente. Esta acción no se puede deshacer.
                        </p>
                    </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async fetchData () {
            ...
            await this.$http.post('api/academic/designations/index', model)
                .then((response) => {
        ...
        async deleteData (id) {
            this.loading = true
            await this.$http.delete('api/academic/designations/' + id)
                .then((response) => {
                    if ( response.status == 201 ) {
                        this.$toast.success("Designación eliminada exitosamente.")
                        this.fetchData()
                    }
                }).catch((response) => {
                    this.$toast.error()
        ...
        showDesignation: function (id) {
            this.$router.push({name: 'designations.show',params:{id:id} })
        }
    }
}
</script>