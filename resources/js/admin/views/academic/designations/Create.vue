                        ...
                        <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                            <div class="flex flex-wrap -mx-3">
                                <div class="w-full px-3">
                                    <input class="appearance-none block w-full bg-gray-100 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight uppercase cursor-default" 
                                        type="text" v-model="data.room" readonly>
                                </div>
                            </div>
                        </dd>
                    </div>
...
export default {
    ...
    methods: {
        ...
        async store () {
            this.loading = true
            await this.$http.post('api/academic/designations', this.model)
                .then((response) => {
                    if ( response.status == 201 ) {
                        this.$toast.success("Designación registrado exitosamente.")
